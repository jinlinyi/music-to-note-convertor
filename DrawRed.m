...
% Draw the wrong music note in red 
...
if note~=-Inf
    ...
    if 16/pace==6 | 16/pace==3 | 16/pace==12
        plot(x+2.5,y+0.5*delta+floor(center)*delta,'.r');
        pace=pace*3/2;
    end
    ...
    if pace>2
        fill(xt,yt,[1 0 0],'Edgecolor','r');
    else
        plot(xt,yt,'color','r','lineWidth',1);
    end
    for i=0:k:fix(center)
        plot([x+length/2-delta,x+length/2+delta],[y+i*delta,y+i*delta],'color','k');
    end
    if pace>1
        plot([x+length/2-0.5*k*delta,x+length/2-0.5*k*delta],[y+(center-k*3.5)*delta,y+center*delta],'color','r');
    end
    ...
    switch pace
        case 1
            fill([x+length/2-0.5*delta,x+length/2-0.5*delta,x+length/2+0.5*delta,x+length/2+0.5*delta],[y,y-0.3*delta,y-0.3*delta,y],[1 0 0],'Edgecolor','r');
        case 2
            fill([x+length/2-0.5*delta,x+length/2-0.5*delta,x+length/2+0.5*delta,x+length/2+0.5*delta],[y,y+0.3*delta,y+0.3*delta,y],[1 0 0],'Edgecolor','r');
        case 4
            C=imread('3.jpg');
            C(C(:,:,1)<200)=255-C(C(:,:,1)<200);
            image([x+1.2 x+1.8],[y+1.3 y-1.3],C);
        case {8,16,32}
            plot([x+length/2-(0.2+0.2*log2(pace/4))*delta,x+length/2+0.4*delta],[y-0.5*(1+log2(pace/4))*delta,y+1*delta],'color','r');
            t=linspace(0,2*pi,100);
            for i=1:1:log2(pace/4)
